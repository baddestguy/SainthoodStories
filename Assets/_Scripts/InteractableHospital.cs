        ...
        switch (button)
        {
            case "BABY":
                ...
                if (MaxDeliveryPoints - DeliveryCountdown == 1)
                    return GameDataManager.Instance.GetToolTip(TooltipStatId.BABY, cpModifier: BabyPoints + (e != null ? (int)e.Gain : 0), energyModifier: -GameManager.Instance.Player.ModifyEnergyConsumption(amount: EnergyConsumption));
                else
